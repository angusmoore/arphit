...
line_y_indices <- function(text,
  ...
  top <- y +
    (0.5 * getstrheight(text, units = "inches") + line_offset) /
    (graphics::par("pin")[2]) * (ylim[2] - ylim[1])
  bottom <- y +
    (- 0.5 * getstrheight(text, units = "inches") + line_offset) /
    (graphics::par("pin")[2]) * (ylim[2] - ylim[1])

  if (line_n == 1)  {
    top <- top + padding / (graphics::par("pin")[2]) * (ylim[2] - ylim[1])
  }
  if (line_n == total_lines) {
    bottom <- bottom - padding / (graphics::par("pin")[2]) * (ylim[2] - ylim[1])
  }

  which(y_start < top & (y_start + y_start[2] - y_start[1]) > bottom)
}

create_text_bitmap <- function(x,
                               y,
                               text,
                               xlim,
                               ylim,
                               ...
                               layout,
                               p,
                               padding = AUTOLABEL_PADDING) {

  x_scale <- graphics::par("mfrow")[2]
  y_scale <- graphics::par("mfrow")[1]

  x_start <- seq(from = xlim[1],
                 to = xlim[2],
                 length.out = round(dim[1] / x_scale) + 1)
  x_start <- x_start[1:round(dim[1] / x_scale)]
  y_start <- seq(from = ylim[2],
                 to = ylim[1],
                 length.out = round(dim[2] / y_scale) + 1)
  y_start <- y_start[2:(round(dim[2] / y_scale) + 1)]

  left <- x - 0.5 * (getstrwidth(text, units = "inches") + padding) /
    (graphics::par("pin")[1]) * (xlim[2] - xlim[1])
  right <- x + 0.5 * (getstrwidth(text, units = "inches") + padding) /
    (graphics::par("pin")[1]) * (xlim[2] - xlim[1])

  x_indices <-
    which(x_start < right & (x_start + x_start[2] - x_start[1]) > left)
  ...
  x_indices <- x_indices + round((dim[1] / x_scale) *
                                   (getlocation(p, layout)[2] - 1))
  y_indices <- y_indices + round((dim[2] / y_scale) *
                                   (getlocation(p, layout)[1] - 1))

  list(x = x_indices, y = c(y_indices, max(y_indices) + 1))
}

shift_text_indices <- function(indices,
                               x,
                               y,
                               x_text_anchor,
                               y_text_anchor,
  ...
  x_shift <- round((x - x_text_anchor) /
                     (xlim[2] - xlim[1]) * (dim[1] / x_scale))
  y_shift <- -round((y - y_text_anchor) /
                      (ylim[2] - ylim[1]) * (dim[2] / y_scale))
...
add_new_label_to_mask <- function(plot_bitmap,
                                  new_label,
                                  ylim,
                                  xlim,
                                  p,
