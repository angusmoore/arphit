...
candidate_from_x_anchor <- function(x, label_indices, x_text_anchor, y_text_anchor, x_scale, y_scale, series, data, xlim, ylim, layout, p, log_scale, underlay_bitmap, los_mask, bars.stacked, quiet, inches_conversion, has_linebreak) {
  if (!quiet) cat(".")
...
find_candidates <- function(label, plot_bitmap, series, data, xlim, ylim, layout, p, log_scale, underlay_bitmap, los_mask, bars.stacked, quiet, inches_conversion) {
...
autolabel_series <- function(label, series, p, data, plot_bitmap, los_mask, xlim, ylim, margins, layout, log_scale, bars.stacked, quiet, inches_conversion, arrow_lines, arrow_bars) {
  ...
  if (is.null(found_location)) {
    found_location <- autolabel_fallback(label$label, xlim[[p]], ylim[[p]], plot_bitmap, layout, p, quiet)
  }
  if (!quiet) cat("\n")
  if (!is.null(found_location)) {
    newlabel <- list(
  ...
  for (label in series_to_label) {
    for (p in label$panels) {
      ...
      if (!is.null(new_label$label)) {
        plot_bitmap <- add_new_label_to_mask(plot_bitmap, new_label, ylim[[p]], xlim[[p]], p, gg$layout)
