...
evaluate_candidate <- function(x,
  ...
  indices <- shift_text_indices(text_indices,
                                x,
                                y,
                                x_text_anchor,
                                y_text_anchor,
  ...
  if (!test_collision(underlay_bitmap, indices$x, indices$y, dim)) {
    ...
    series_distances <- lapply(seq_along(data$series),
                               function(s)
                                 get_distance_series_type(
                                   x,
                                   y,
                                   get_x_plot_locations(
                                     series_x_values(data, s),
                                     data),
    ...
    distance$los <- lineofsight(distance$xx,
                                distance$yy,
                                x,
                                y,
                                los_mask,
    ...
    if (length(next_closest) == 1) {
      ...
      distance$next_closest <- Inf
      # Happens for panels with LHS/RHS axes if there
    ...
    distance$selection_group <- assign_selection_group(distance$distance,
                                                       distance$next_closest,
                                                       distance$los)
    return(distance)
  } else {
    return(NULL)
...
divide_y_axis <- function(ylim, divisor, axis_points, axis_step, remove_axis) {
    new_points <- seq(from = ylim[1] + axis_step / divisor,
                      by = axis_step / divisor,
                      to = ylim[2] - axis_step / divisor)
    if (remove_axis) {
...
autolabeller_y_points <- function(ylim, ylim_nsteps, has_linebreak) {
  ...
  axis_points <- seq(from = ylim[1] + axis_step,
                     by = axis_step,
                     to = ylim[2] - axis_step)

  for (divisor in seq(from = 2, by = 2, to = AUTOLABEL_YSTEPS)) {
    candidate_points <- divide_y_axis(ylim,
                                      divisor,
                                      axis_points,
                                      axis_step,
                                      !has_linebreak)
...
candidate_from_x_anchor <- function(x,
                                    label_indices,
                                    x_text_anchor,
                                    y_text_anchor,
                                    x_scale,
...
find_candidates <- function(label,
                            plot_bitmap,
                            series,
                            data,
                            xlim,
  ...
  x_anchors <- seq(from = xlim[1] + 1.5 * step,
                   by = step,
                   length.out = AUTOLABEL_XSTEPS - 2)

  x_text_anchor <- mean(xlim)
  y_text_anchor <- mean(ylim)
  x_scale <- graphics::par("mfrow")[2]
  y_scale <- graphics::par("mfrow")[1]
  text_indices <- create_text_bitmap(x_text_anchor,
                                     y_text_anchor,
                                     label$label,
                                     xlim,
                                     ylim,
...
autolabel_fallback <- function(label,
                               xlim,
                               ylim,
                               underlay_bitmap,
                               layout,
                               p,
                               quiet) {
  x_up <- seq(from = mean(xlim), to = max(xlim),
              length.out = AUTOLABEL_FALLBACK_STEPS)
  x_down <- seq(from = mean(xlim), to = min(xlim),
                length.out = AUTOLABEL_FALLBACK_STEPS)
  y_up <- seq(from = mean(ylim), to = max(ylim),
              length.out = AUTOLABEL_FALLBACK_STEPS)
  y_down <- seq(from = mean(ylim), to = min(ylim),
                length.out = AUTOLABEL_FALLBACK_STEPS)
  ...
  for (x in x_steps) {
    ...
    for (y in y_steps) {
      indices <- create_text_bitmap(x,
                                    ...
                                    layout,
                                    p,
                                    padding = AUTOLABEL_FALLBACK_PADDING)
      if (!test_collision(underlay_bitmap, indices$x, indices$y, dim)) {
        return(data.frame(
          x = x,
          y = y,
...
autolabel_series <- function(label,
                             series,
                             p,
                             data,
                             plot_bitmap,
  ...
  if (is.null(found_location)) {
    found_location <- autolabel_fallback(label$label,
                                         xlim[[p]],
                                         c(ylim[[p]]$min, ylim[[p]]$max),
                                         plot_bitmap,
                                         layout,
  ...
  if (!is.null(found_location)) {
    ...
    drawlabel(newlabel)
    if ((series$geomtype != "bar" && series$geomtype != "waterfall") &&
        arrow_lines) {
      return(list(
        label = newlabel,
        arrow = add_arrow(found_location,
                          newlabel$text,
                          label$col,
                          p,
                          inches_conversion)
      ))
    } else if ((series$geomtype == "bar" || series$geomtype == "waterfall") &&
               arrow_bars) {
      return(list(
        label = newlabel,
        arrow = add_arrow(
...
autolabel <- function(gg,
  ...
  inches_conversion <- list(
    y = 1 / (graphics::par("usr")[4] - graphics::par("usr")[3]) *
      graphics::par("pin")[2],
    x = 1 / (graphics::par("usr")[2] - graphics::par("usr")[1]) *
      graphics::par("pin")[1]
  )
  series_to_label <- createlabels(data,
                                  gg$layout,
                                  labels,
                                  ylim,
                                  ignore_existing_labels)
  ...
  for (label in series_to_label) {
    for (p in label$panels) {
      # Create LOS mask
      los_mask <- create_los_mask(label$series,
                                  data,
                                  p,
                                  dim(plot_bitmap),
                                  xlim,
      ...
      if (!is.null(new_label$label)) {
        plot_bitmap <- add_new_label_to_mask(plot_bitmap,
                                             new_label,
                                             c(ylim[[p]]$min, ylim[[p]]$max),
                                             xlim[[p]],
                                             p,
