...
```  
 ...
 10. One panel horizontal. This layout has only 1 panel - there are not top and 
 bottom y axes.
```{r, eval = FALSE}
...
closest to the axis) using the argument `reorder_bars` to `agg_col`. This is covered
in detail under the `agg_col` section of the section 'Layers' below.
...
You can use the layout option `layout = "1h"` to create horizontal bar graphs - 
...
the bars _within_ a layer are ordered alphabetically;^[Series without a name (i.e.
value of grouping variable equal to `NA`) appear last.] but later layers always
...
You can manually control the order of bars using the argument `reorder_bars`. All bar series
added to a panel up to that point can be reordered within that layer. As such, you
should always reorder in the _last_ layer you add to a panel:
...
You do not have to specify a full order. If you leave out series, these are ordered
after those you manually order (in alphabetical order). You will get a warning telling
you this (in case it was a mistake).
...
You can rename series using the function `agg_rename_series`. See the section on
...
unless you set `ignore_existing_labels = TRUE` as an argument to `agg_autolabel`:
```{r, results = "hide"}
...
vertical lines are similar, but the `x` variable only. You can also control the
colour and line type. These are optional and default to a solid black line. You
must also tell `arphit` which panel to put the line on.
...
If you need more control over the line, you can use `agg_abline` to specify the
coordinates of the line segment instead of using `x` or `y`.  To do so use
`x1`, `x2`, `y1` and `y2`. All other options are the same. For example:
```{r, results = "hide"}
p <- arphitgg() + 
...
By default, series names in legends are the name of the `y` aesthetic (if the
...
`agg_rename_series` exists to allow you to easily rename series for legends (or
...
structure as `dplyr`'s `rename` function.) You can optional restrict the mapping
...
name to `NA`. This is a little complicated in `agg_rename_series` because R does not
accept NA as a name in a list. Instead use the string "<NA>" as the series name 
to indicate that a series should not be displayed in a legend:
```{r, results = "hide"}
p <- arphitgg(long_data, agg_aes(x = date, y = y1, group = group_var)) +
  agg_line() +
