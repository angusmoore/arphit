...
`agg_point` layers are just a wrapper around `agg_line` that set the line to NA, and the point marker (`pch`) to `19` for solid dots. Naturally, these are best for scatter graphs, as they quicker and easier than constructing your own `agg_line` layer.

`agg_point` has only four arguments. Data (`data`), aesthetic (`aes`), panel (`panel`) and colour for the markers (`color`). As with other layer types, data and aesthetic (including parts of an aesthetic) will be inherited from the parent if omitted.
...
```{r, results = "hide"}
p <- arphitgg(scatterdata, agg_aes(x=x,y=y)) + 
...
```{r, results = "hide"}
p <- arphitgg(simple_data) + 
  agg_line(agg_aes(x=x,y=y1),panel="1") + 
  agg_line(agg_aes(x=x,y=y1),panel="2") + 
...
```{r, results = "hide"}
p <- arphitgg() + 
  agg_bgshading(x1 = NA< y1 = -1, x2 = NA, y2 = 3, panel = "1")
```
```{r, out.width = "50%", echo = FALSE, fig.align="center"}
...
```{r, echo = FALSE, results = "hide"}
arphitgg(long_data, agg_aes(x = date, y = y1, group = group_var)) + agg_line() + agg_legend()
```
```{r, out.width = "50%", echo = FALSE, fig.align="center"}
