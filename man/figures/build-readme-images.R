...
agg_draw(p, filename = "README-simple_example.png")
...
agg_draw(p, filename = "README-complex_example.png")

tsdata <- ts(data.frame(y=rnorm(10)), frequency = 4, start = 200)
agg_qplot(tsdata, filename = "README-qplot.png")

p <- arphitgg(data, agg_aes(x=date, y = x1)) + agg_line()
agg_draw(p, filename = "README-nooptions.png")
...
agg_draw(p, filename = "README-lotsofoptions.png")
