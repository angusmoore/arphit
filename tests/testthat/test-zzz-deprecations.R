test_that("Deprecation warnings for color", {
  ...
  expect_warning(
    arphitgg() +
      agg_label(x = 1, y = 2, text = "a", panel = "1", color = "red"),
    "color is deprecated; use colour instead",
    fixed = TRUE
  )
  ...
  expect_warning(
    arphitgg() +
      agg_arrow(tail.x = 1, tail.y = 2, head.x = 1, head.y = 3,
                panel = "1", color = "red"),
    "color is deprecated; use colour instead",
    fixed = TRUE
  )
  ...
  expect_warning(
    arphitgg() +
      agg_bgshading(x1 = 1, y1 = 2, x2 = 2, y2 = 3, panel = "1", color = "red"),
    "color is deprecated; use colour instead",
    fixed = TRUE
  )
  ...
  expect_warning(
    arphitgg(data) +
      agg_line(agg_aes(x = x, y = y)) + agg_line(agg_aes(x = x, y = z)) +
      agg_shading(from = z, to = y, color = "red"),
    "color is deprecated; use colour instead",
    fixed = TRUE
  )
})
